...
select problem,
       ...
       percentile_cont(0.5) within group ( order by evaluate ) / 1000 as evaluate,
       percentile_cont(0.5) within group ( order by copy ) / 1000     as copy,
       percentile_cont(0.5) within group ( order by match ) / 1000    as match,
       percentile_cont(0.5) within group ( order by mutate ) / 1000   as mutate
from run
...
ggplot(df, aes(x = model, y = value, fill = representation)) +
  ...
  labs(x = "Instance", y = "Duration (µs)", fill = "Representation") +
